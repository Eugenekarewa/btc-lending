...
interface LoanData {
  // Define loan data structure as needed
}
...
const App: React.FC = () => {
  ...
  const handleWalletConnect = (wallet: WalletData) => {
    setWalletData(wallet);
    setCurrentStep('lock');
  };

  const handleLockComplete = (lock: LockData) => {
    setLockData(lock);
    setCurrentStep('borrow');
  };

  const handleLoanRequest = (loan: LoanData) => {
    setLoanData(loan);
